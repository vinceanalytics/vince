...
func Pipe(ctx context.Context) plug.Pipeline {
	...
	return plug.Pipeline{
		...
		plug.PREFIX("/stats/property",
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/property/series/:owner/:site$", stats.PropertySeries),
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/property/aggregate/:owner/:site$", stats.PropertyAggregate),
			NotFound,
		),
		...
		plug.PREFIX("/stats/",
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/:owner$", stats.All),
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/:owner/:metric$", stats.Metric),
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/:owner/:site$", stats.All),
			a.And(plug.AuthAPI(schema.Stats, schema.Get)).
				GET("^/stats/:owner/:site/:metric$", stats.Metric),
