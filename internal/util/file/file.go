...
var ErrCreatingNewFile = errors.New("util/file: creating a new file")
...
func OpenMmapFileUsing(fd *os.File, sz int, writable bool) (*MmapFile, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("while mmapping %s with size: %d%w", fd.Name(), fileSize, err)
	}
...
func OpenMmapFile(filename string, flag, maxSz int) (*MmapFile, error) {
	...
	fd, err := os.OpenFile(filename, flag, 0o666)
	if err != nil {
...
func (m *MmapFile) Delete() error {
	...
	if err := Munmap(m.Data); err != nil {
		return fmt.Errorf("while munmap file: %s, error: %w", m.Fd.Name(), err)
	}
	m.Data = nil
	if err := m.Fd.Truncate(0); err != nil {
		return fmt.Errorf("while truncate file: %s, error: %w", m.Fd.Name(), err)
	}
	if err := m.Fd.Close(); err != nil {
		return fmt.Errorf("while close file: %s, error: %w", m.Fd.Name(), err)
	}
...
func (m *MmapFile) Close(maxSz int64) error {
	...
	if err := m.Sync(); err != nil {
		return fmt.Errorf("while sync file: %s, error: %w", m.Fd.Name(), err)
	}
	if err := Munmap(m.Data); err != nil {
		return fmt.Errorf("while munmap file: %s, error: %w", m.Fd.Name(), err)
	}
