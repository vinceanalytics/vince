...
// Store implements the SnapshotStore interface and allows
...
// New creates a new FileSnapshotStore based
...
func (f *Store) testPermissions() error {
	...
	sink := &FileSnapshotSink{
		...
		meta: fileSnapshotMeta{
			SnapshotMeta: raft.SnapshotMeta{
				...
				Index:              index,
				Term:               term,
				Peers:              encodePeers(&configuration),
...
func encodePeers(x *raft.Configuration) []byte {
	var ls v1.Server_List
	ls.Items = make([]*v1.Server, len(x.Servers))
	for i := range x.Servers {
		ls.Items[i] = &v1.Server{
			Id:   string(x.Servers[i].ID),
			Addr: string(x.Servers[i].Address),
		}
	}
...
func (f *Store) getSnapshots() ([]*fileSnapshotMeta, error) {
	// Get the eligible snapshots
...
func (f *Store) readMeta(name string) (*fileSnapshotMeta, error) {
	// Open the meta file
