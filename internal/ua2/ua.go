...
var (
	...
	botsCache = map[uint64]struct{}{}

	mu sync.RWMutex
)

func Parse(s string, m *models.Model) (isBot bool) {
	hash := hash.String(s)
	mu.RLock()
	if _, ok := botsCache[hash]; ok {
		mu.RUnlock()
		return true
	}
	a, ok := cache[hash]
	...
	defer mu.Unlock()
	{
		...
		match, _ := allBotsReStandardMatch().MatchString(s)
		if match {
			botsCache[hash] = struct{}{}
			return true
		}
	}
	parseUA(s, m)
