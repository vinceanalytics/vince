...
compilefile(relPath('src/plausible.js'), relPath('../app/public/js/len64.js'))

variants.map(variant => {
  const options = variant.map(variant => variant.replace('-', '_')).reduce((acc, curr) => (acc[curr] = true, acc), {})
  compilefile(relPath('src/plausible.js'), relPath(`../app/public/js/len64.${variant.join('.')}.js`), options)
})
