...
func main() {
	println("### Generating regex for ua clients ###")
	make()
}
...
func genBrowserEngine(b *bytes.Buffer) {
	generic(b, "BrowserEngine", "client/browser_engine.yml")
}

func genBrowser(b *bytes.Buffer) {
	generic(b, "Browser", "client/browsers.yml")
}
func genFeedReader(b *bytes.Buffer) {
	generic(b, "FeedReader", "client/feed_readers.yml")
}

func genLibraries(b *bytes.Buffer) {
	generic(b, "Library", "client/libraries.yml")
}

func genMediaPlayers(b *bytes.Buffer) {
	generic(b, "MediaPlayer", "client/mediaplayers.yml")
}
func genMobileApps(b *bytes.Buffer) {
	generic(b, "MobileApp", "client/mobile_apps.yml")
}
func genPim(b *bytes.Buffer) {
	generic(b, "Pim", "client/pim.yml")
}

func generic(b *bytes.Buffer, name string, path string) {
	var items []*ClientReg
	tools.ReadUA(path, &items)
